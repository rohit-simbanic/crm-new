...
export const escrow_company_email_c = {
  validate: function (oppurtunity: any, status: string, oldOppurtunity: any) {
    ...
    if (
      ...
      (!oppurtunity?.escrow_company_email_c ||
        oppurtunity?.escrow_company_email_c.trim().length === 0)
    ) {
      errors.push(missingField('Escrow Company Email'));
    }
