...
export const commission_reduction_type3 = {

    validate: (opportunity: ObjectType, status: string, originalOpportunity: ObjectType) => {

        let errors = []
        if (!isEmpty(opportunity?.commision_reduction3) && isEmpty(opportunity?.commission_reduction_type3)) {
            errors.push(missingField("Reduction Type3"))
        }
