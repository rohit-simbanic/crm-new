...
const PreOfferProperties = () => {
  ...
  const getProperties = async () => {
    if (isEmpty(filterService(filter, 'properties'))) return;
  ...
  useEffect(() => {
    eventBus.on(`${DASHBOARDS.PROPERTIES_PRE_OFFERS}_refresh_columns`, () => {
      udpateRefreshColumn();
    });
  }, []);
  ...
  return (
    <>
      <Filter
        ...
        showAll={showAll}
        currentTab={DASHBOARDS.PROPERTIES_PRE_OFFERS}
      />
      <DataGrid
        rows={properties}
        columns={getColumns()}
...
export default PreOfferProperties;
