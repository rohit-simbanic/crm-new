...
const Filter = ({
  ...
  const loadAll = () => {
    ...
    for (const el of getObjectEntriesAsArray(propertiesClosingStateOfClosing)) {
      ...
      <PaperBoxContent>
        <FormContainer>
          <AutoCompleteUnit
            ...
            options={getObjectEntriesAsArray(propertiesClosingStateOfClosing)}
            multiple={true}
          ...
          <UnitItem grid={{ xs: 12, sm: 4 }}>
            <FormContainer spacing={1}>
              <UnitSelect
                name="close_date_c_range"
                label={fieldLabel.dateRange}
                records={getObjectEntriesAsArray(dateRange)}
                value={filter.close_date_c_range ?? ''}
                onChange={(e: any) => updateFilter(e.target)}
                readOnly={false}
                grid={{ xs: 12, sm: 4 }}
              />
              {filter.close_date_c_range === 'date-range' ? (
                <>
                  <UnitDate
                    label={fieldLabel.closeDate}
                    name="close_date_c"
                    value={filter.close_date_c ?? ''}
                    onChange={(e: any) => {
                      updateFilter({
                        name: 'close_date_c',
                        value: e
                  ...
                  <UnitDate
                    label={fieldLabel.closeToDate}
                    name="close_date_to_c"
                    value={filter.close_date_to_c ?? ''}
                    onChange={(e: any) => {
                      updateFilter({
                        name: 'close_date_to_c',
                        value: e
