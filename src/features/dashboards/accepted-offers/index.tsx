...
import initialFilter from 'state/dashboard/accepted-offers/initial-filter';
...
const AcceptedOffers = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.NEGOTIATOR_DASHBOARD_ACCEPTED_OFFER
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      acceptedOffer: result.data.meta.total
    });
  };
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_ACCEPTED_OFFER}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_ACCEPTED_OFFER}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.NEGOTIATOR_DASHBOARD_ACCEPTED_OFFER
        }}
      >
