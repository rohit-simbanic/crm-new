...
import initialFilter from 'state/dashboard/option-fee/initial-filter';
...
const OptionFee = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.TRANSACTION_DASHBOARD_OPTION_FEE
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      optionFee: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.TRANSACTION_DASHBOARD_OPTION_FEE,
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_OPTION_FEE}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_OPTION_FEE}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_OPTION_FEE
        }}
      >
