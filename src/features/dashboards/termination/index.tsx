...
import initialFilter from 'state/dashboard/termination/initial-filter';
...
const TerminationOffers = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.NEGOTIATOR_DASHBOARD_TERMINATION
  );
  ...
  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.NEGOTIATOR_DASHBOARD_TERMINATION,
  ...
  useEffect(() => {
    ...
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_TERMINATION}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.NEGOTIATOR_DASHBOARD_TERMINATION
        }}
      >
...
export default TerminationOffers;
