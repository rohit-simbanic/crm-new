...
import initialFilter from 'state/dashboard/termination-pending/initial-filter';
...
const TerminationPending = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.TRANSACTION_DASHBOARD_TERMINATION_PENDING
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      terminationPending: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.TRANSACTION_DASHBOARD_TERMINATION_PENDING,
  ...
  useEffect(() => {
    ...
    if (!initialLoad && currentTab !== 6) return;
  ...
  useEffect(() => {
    if (currentTab !== 6 || initialLoad) return;
  ...
  useEffect(() => {
    eventBus.on(
      `$${DASHBOARDS.TRANSACTION_DASHBOARD_TERMINATION_PENDING}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_TERMINATION_PENDING}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_TERMINATION_PENDING
        }}
      >
...
export default TerminationPending;
