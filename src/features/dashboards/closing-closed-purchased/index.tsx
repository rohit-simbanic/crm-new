...
import initialFilter from 'state/dashboard/closing-closed-purchased/initial-filter';
...
const ClosedPurchased = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.CLOSING_DASHBOARD_CLOSED_PURCHASED
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      purchased: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.CLOSING_DASHBOARD_CLOSED_PURCHASED,
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_CLOSED_PURCHASED}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_CLOSED_PURCHASED}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.CLOSING_DASHBOARD_CLOSED_PURCHASED
        }}
      >
...
export default ClosedPurchased;
