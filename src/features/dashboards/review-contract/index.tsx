...
import initialFilter from 'state/dashboard/review-contract/initial-filter';
...
const ReviewContract = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.TRANSACTION_DASHBOARD_REVIEW_CONTRACT
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      reviewContract: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.TRANSACTION_DASHBOARD_REVIEW_CONTRACT,
  ...
  useEffect(() => {
    ...
    if (!initialLoad && currentTab !== 3) return;
  ...
  useEffect(() => {
    if (currentTab !== 3 || initialLoad) return;
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_REVIEW_CONTRACT}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_REVIEW_CONTRACT}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_REVIEW_CONTRACT
        }}
      >
