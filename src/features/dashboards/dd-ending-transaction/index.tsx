...
import initialFilter from 'state/dashboard/dd-ending-transaction/initial-filter';
...
const DDEnding = ({
  ...
  useEffect(() => {
    eventBus.on(`${DASHBOARDS.TRANSACTION_DASHBOARD_DD_ENDING}_refresh`, () => {
      updateRefresh();
    });
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_DD_ENDING}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_DD_ENDING
        }}
      >
