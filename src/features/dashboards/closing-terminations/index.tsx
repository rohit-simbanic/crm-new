...
import initialFilter from 'state/dashboard/closing-terminations/initial-filter';
...
const Terminations = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.CLOSING_DASHBOARD_TERMINATIONS
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      termination: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.CLOSING_DASHBOARD_TERMINATIONS,
  ...
  useEffect(() => {
    eventBus.on(`${DASHBOARDS.CLOSING_DASHBOARD_TERMINATIONS}_refresh`, () => {
      updateRefresh();
    });
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_TERMINATIONS}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.CLOSING_DASHBOARD_TERMINATIONS
        }}
      >
