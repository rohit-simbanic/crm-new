...
import initialFilter from 'state/dashboard/clear-to-close/initial-filter';
...
const ClearToClose = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.CLOSING_DASHBOARD_CLEAR_TO_CLOSE
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      clearToClose: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.CLOSING_DASHBOARD_CLEAR_TO_CLOSE,
  ...
  useEffect(() => {
    ...
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_CLEAR_TO_CLOSE}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.CLOSING_DASHBOARD_CLEAR_TO_CLOSE
        }}
      >
