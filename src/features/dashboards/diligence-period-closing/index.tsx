...
import initialFilter from 'state/dashboard/diligence-period-closing/initial-filter';
...
const DiligenceperiodClosing = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.CLOSING_DASHBOARD_DILIGENCE_PERIOD
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      diligencePeriod: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.CLOSING_DASHBOARD_DILIGENCE_PERIOD,
  ...
  useEffect(() => {
    ...
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_DILIGENCE_PERIOD}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.CLOSING_DASHBOARD_DILIGENCE_PERIOD
        }}
      >
...
export default DiligenceperiodClosing;
