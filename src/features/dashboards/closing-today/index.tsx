...
import initialFilter from 'state/dashboard/closing-today/initial-filter';
...
const ClosingToday = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.TRANSACTION_DASHBOARD_CLOSING_TODAY
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      closingToday: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.TRANSACTION_DASHBOARD_CLOSING_TODAY,
  ...
  useEffect(() => {
    ...
    if (!initialLoad && currentTab !== 5) return;
  ...
  useEffect(() => {
    if (currentTab !== 5 || initialLoad) return;
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_CLOSING_TODAY}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_CLOSING_TODAY}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_CLOSING_TODAY
        }}
      >
