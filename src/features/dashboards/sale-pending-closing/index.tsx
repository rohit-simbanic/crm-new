...
import initialFilter from 'state/dashboard/sale-pending-closing/initial-filter';
...
const SalePendingClosing = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.CLOSING_DASHBOARD_SALE_PENDING
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      salePending: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.CLOSING_DASHBOARD_SALE_PENDING,
  ...
  useEffect(() => {
    eventBus.on(`${DASHBOARDS.CLOSING_DASHBOARD_SALE_PENDING}_refresh`, () => {
      updateRefresh();
    });
    eventBus.on(
      `${DASHBOARDS.CLOSING_DASHBOARD_SALE_PENDING}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.CLOSING_DASHBOARD_SALE_PENDING
        }}
      >
...
export default SalePendingClosing;
