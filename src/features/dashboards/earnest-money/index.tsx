...
import initialFilter from 'state/dashboard/earnest-money/initial-filter';
...
const EarnestMoney = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.TRANSACTION_DASHBOARD_EARNEST_MONEY
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      earnestMoney: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.TRANSACTION_DASHBOARD_EARNEST_MONEY,
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_EARNEST_MONEY}_refresh`,
      () => {
        updateRefresh();
      }
    );
    ...
    eventBus.on(
      `${DASHBOARDS.TRANSACTION_DASHBOARD_EARNEST_MONEY}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.TRANSACTION_DASHBOARD_EARNEST_MONEY
        }}
      >
