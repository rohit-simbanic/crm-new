...
import initialFilter from 'state/dashboard/counters/initial-filter';
...
const CounterOffers = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.NEGOTIATOR_DASHBOARD_COUNTERED_OFFER
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      counterdOffer: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.NEGOTIATOR_DASHBOARD_COUNTERED_OFFER,
  ...
  useEffect(() => {
    ...
    if (!initialLoad && currentTab !== 2) return;
  ...
  useEffect(() => {
    if (currentTab !== 2 || initialLoad) return;
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_COUNTERED_OFFER}_countered_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_COUNTERED_OFFER}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.NEGOTIATOR_DASHBOARD_COUNTERED_OFFER
        }}
      >
