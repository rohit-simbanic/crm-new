...
import initialFilter from 'state/dashboard/seller-sent-received/initial-filter';
...
const SellerSentReceived = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.NEGOTIATOR_DASHBOARD_SELLER_SENT_RECEIVED
  );
  ...
  const getOpportunities = async () => {
    ...
    updateCount({
      sellerReceived: result.data.meta.total
    });
  };

  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.NEGOTIATOR_DASHBOARD_SELLER_SENT_RECEIVED,
  ...
  useEffect(() => {
    ...
    if (!initialLoad && currentTab !== 1) return;
  ...
  useEffect(() => {
    if (currentTab !== 1 || initialLoad) return;
  ...
  useEffect(() => {
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_SELLER_SENT_RECEIVED}_refresh`,
      () => {
        updateRefresh();
      }
    );
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_SELLER_SENT_RECEIVED}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.NEGOTIATOR_DASHBOARD_SELLER_SENT_RECEIVED
        }}
      >
...
export default SellerSentReceived;
