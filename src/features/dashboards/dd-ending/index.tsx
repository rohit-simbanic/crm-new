...
import initialFilter from 'state/dashboard/dd-ending/initial-filter';
...
const DDEndingOffers = ({
  ...
  const [selectedColumns, setSelectedColumns] = useState<string[]>(
    DefaultDashboardFields.NEGOTIATOR_DASHBOARD_DD_ENDING
  );
  ...
  const getTableColumnsToShow = async () => {
    const response = await userPreferenceService.getItem({
      category: DASHBOARDS.NEGOTIATOR_DASHBOARD_DD_ENDING,
  ...
  useEffect(() => {
    eventBus.on(`${DASHBOARDS.NEGOTIATOR_DASHBOARD_DD_ENDING}_refresh`, () => {
      updateRefresh();
    });
    eventBus.on(
      `${DASHBOARDS.NEGOTIATOR_DASHBOARD_DD_ENDING}_refresh_columns`,
      () => {
        udpateRefreshColumn();
      }
    );
    ...
    <React.Fragment>
      <DashboardContext.Provider
        value={{
          ...
          currentTab: DASHBOARDS.NEGOTIATOR_DASHBOARD_DD_ENDING
        }}
      >
